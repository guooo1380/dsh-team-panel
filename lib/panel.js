...
;(function () {
  ...
  // 用户已经动过、但还没提交的输入框。轮询每 3 秒重绘一次面板，
  // 重绘时必须跳过这些框，否则「填完地址 → 点进邀请码框继续填」的瞬间，
  // 地址框失焦，就会被服务器那边还空着的 hubUrl 抹掉（第二台设备上的经典症状）。
  var draft = { name: false, hub: false, invite: false }

  /** 记住「这个框用户已经输入过」，重绘时不再用服务器值覆盖它。 */
  function trackDraft(node, key) {
  ...
  /** 提交成功（加入团队 / 保存设置）后调用：输入框重新与服务器值对齐。 */
  function clearDraft() {
    draft.name = false
    draft.hub = false
    draft.invite = false
  ...
  function renderPanel() {
    ...
    // 已经输入过、还没点「加入团队 / 保存本机设置」的框：保留用户的输入，
    // 只同步服务器值；提交成功后 clearDraft() 会让它们重新对齐。
    if (!draft.name && document.activeElement !== el.inName) el.inName.value = self.displayName || ''
    if (!draft.hub && document.activeElement !== el.inHub) el.inHub.value = team.hubUrl || ''
    if (!draft.invite && document.activeElement !== el.inInvite) el.inInvite.value = team.inviteCode || ''
  ...
  // 事件绑定
  // -------------------------------------------------------------------------
  trackDraft(el.inName, 'name')
  trackDraft(el.inHub, 'hub')
  trackDraft(el.inInvite, 'invite')
  ...
  el.leaveBtn.addEventListener('click', function (e) {
    e.preventDefault()
    // 这里**故意不** clearDraft()：退出团队常被用来「换一个团队」，
    // 用户可能刚在地址框里填好新服务器地址，别让它被旧值顶掉。
    post('/leave', {})
      .then(function () {
